  ...
  /* Infinite loop */
	...
	while (1)
	{
		...
		if(IRReceiver_Read(&dev,&data) == 0){
			if(data == 0xa8){	//play button
				/* 创建播放音乐的任务*/
				extern void PlayMusic(void *parm);
				if(xSoundTaskHandle == NULL){
					ret = xTaskCreate(PlayMusic, "MusicTask", 128, NULL, osPriorityNormal+1,&xSoundTaskHandle);
					LCD_ClearLine(0,0);
					LCD_PrintString(0,0,"Create task");
				}
			}
			
			else if(data == 0xa2){	//power button
				/*删除播放音乐的任务 */
				if(xSoundTaskHandle != NULL){
					...
					LCD_ClearLine(0,0);
					LCD_PrintString(0,0,"Delete success");
				}
			}
		}
	}
